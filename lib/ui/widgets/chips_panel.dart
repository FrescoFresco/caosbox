...
class FilterSet {
  ...
  void setDefaults(Map<FilterKey, FilterMode> d){ clear(); d.forEach((k,v)=>modes[k]=v); }
  void cycle(FilterKey k){ modes[k]=FilterMode.values[(modes[k]!.index+1)%3]; }
  void clear(){ text.clear(); for(final k in modes.keys){ modes[k]=FilterMode.off; } }
  bool get hasActive => text.text.isNotEmpty || modes.values.any((m)=>m!=FilterMode.off);
}

class FilterEngine {
  static bool _pass(FilterMode m,bool v)=>switch(m){FilterMode.off=>true,FilterMode.include=>v,FilterMode.exclude=>!v};
  static List<Item> apply(List<Item> items,AppState s,FilterSet set){
    final q=set.text.text.toLowerCase(); final hasQ=q.isNotEmpty;
...
class ChipsPanel extends StatelessWidget{
  final FilterSet set; final VoidCallback onUpdate; final Map<FilterKey,FilterMode>? defaults;
  ...
  Widget build(BuildContext ctx){
    Widget chip(FilterKey k,String label){
      ...
      final col=on?(m==FilterMode.include?Colors.green:Colors.red).withOpacity(0.3):null;
      final txt=m==FilterMode.exclude?'⊘$label':label;
      return FilterChip(label:Text(txt), selected:on, selectedColor:col, onSelected:(_){ set.cycle(k); onUpdate(); });
    }
    return Column(mainAxisSize:MainAxisSize.min, children:[
      TextField(
        controller:set.text, onChanged:(_)=>onUpdate(),
