...
class Token {
  ...
  factory Token.fromJson(Map<String, dynamic> j) =>
      Token(j['t'] as String, switch (j['mode']) { 'include' => Tri.include, 'exclude' => Tri.exclude, _ => Tri.off });
}
...
class TextClause implements Clause {
  ...
  @override Map<String, dynamic> toJson() => {
        'kind': 'text',
        'fields':
            fields.map((k, v) => MapEntry(k, switch (v) { Tri.include => 'include', Tri.exclude => 'exclude', _ => 'off' })),
