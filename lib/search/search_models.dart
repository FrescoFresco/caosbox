...
class Token {
  ...
  Map<String, dynamic> toJson() => {
        't': t,
        'mode': switch (mode) { Tri.include => 'include', Tri.exclude => 'exclude', _ => 'off' }
      };

  factory Token.fromJson(Map<String, dynamic> j) => Token(
        j['t'] as String,
        switch (j['mode']) { 'include' => Tri.include, 'exclude' => Tri.exclude, _ => Tri.off },
      );
...
class TextClause implements Clause {
  final Map<String, Tri> fields; // id/content/note
  ...
  Map<String, dynamic> toJson() => {
        'kind': 'text',
        'fields': fields.map(
          (k, v) => MapEntry(k, switch (v) { Tri.include => 'include', Tri.exclude => 'exclude', _ => 'off' }),
        ),
...
class SearchSpec {
  ...
  factory SearchSpec.fromJson(Map<String, dynamic> j) {
    ...
    for (final raw in (j['clauses'] as List? ?? const [])) {
      ...
      switch (m['kind']) {
        case 'enum':
          out.add(EnumClause.fromJson(m));
          break;
        case 'text':
          out.add(TextClause.fromJson(m));
          break;
      }
    }
