...
String exportDataJson(AppState st) {
  final data = {
    ...
    'version': 1,
    'meta': {
      'counters': { 'idea': st.counters[ItemType.idea] ?? 0, 'action': st.counters[ItemType.action] ?? 0 },
    ...
    'notes': { for (final it in st.all) if (st.note(it.id).isNotEmpty) it.id: st.note(it.id) },
...
void importDataJsonReplace(AppState st, String text) {
  ...
  for (final k in n.keys) { notes['$k'] = '${n[k]}'; }
...
String exportQueryJson(SearchSpec s) {
  Map<String,dynamic> clauseToMap(Clause c) {
    if (c is EnumClause) {
      return { 'type':'enum', 'field':c.field, 'include':c.include.toList(), 'exclude':c.exclude.toList() };
  ...
  for (final e in (m['clauses'] as List? ?? [])) {
    if (e['type']=='enum') {
      ...
      for (final k in fm.keys) {
        final v = '${fm[k]}';
        fields['$k'] = switch (v) { 'include'=>Tri.include, 'exclude'=>Tri.exclude, _=>Tri.off };
      }
      final toks = <Token>[
        for(final t in (e['tokens'] as List? ?? []))
          Token('${t['t']}', switch('${t['mode']}'){ 'exclude'=>Tri.exclude, 'include'=>Tri.include, _=>Tri.off })
      ];
      clauses.add(TextClause(fields: fields, tokens: toks));
    }
  }
