...
import 'package:caosbox/searchkit/io.dart';
import 'package:caosbox/searchkit/models.dart';

class FiltersSheet extends StatefulWidget {
  final SearchController controller;
  const FiltersSheet({super.key, required this.controller});
...
class _FiltersSheetState extends State<FiltersSheet> {
  final _items = <_Entry>[]; // secuencia lineal con conector antes del bloque (null en primero)

  @override
  void initState() {
    super.initState();
    _fromTree(widget.controller.spec.root);
  }
    ...
    void dfs(QueryNode node, {Op? incoming}) {
      ...
      if (node is GroupNode && node.children.isNotEmpty) {
        ...
        for (int i = 1; i < node.children.length; i++) {
          ...
          if (i == 1) dfs(acc, incoming: incoming);
          dfs(nxt, incoming: op);
          acc = GroupNode(op: op, children: [acc, nxt]);
        }
      }
    }
    dfs(root);
  ...
  GroupNode _toTree() {
    if (_items.isEmpty) return const GroupNode(op: Op.and, children: []);
  ...
  void _apply() {
    widget.controller.setSpec(SearchSpec(root: _toTree()));
  ...
  Widget _textEditor(TextClause c, void Function(TextClause) onChange) {
    ...
    void setElement(String v) {
      element = v;
      if (element == 'id') presence = Tri.off; // presence no aplica a id
